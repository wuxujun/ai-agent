...
func TestValidateBrainCompilerSettingsWhenDisabled(t *testing.T) {
	...
	for _, mutate := range []func(*Config){
		func(cfg *Config) { cfg.Brain.CompactIndexMaxBytes = -1 },
		func(cfg *Config) { cfg.Brain.Compiler.Provider = "openai" },
		func(cfg *Config) { cfg.Brain.Compiler.Model = "" },
		func(cfg *Config) { cfg.Brain.Compiler.MaxInputBytes = -1 },
		func(cfg *Config) { cfg.Brain.Compiler.MaxOutputTokens = -1 },
		func(cfg *Config) { cfg.Brain.Compiler.MaxCostUSD = math.NaN() },
	} {
		cfg := valid()
		mutate(cfg)
		if err := cfg.Validate(); err == nil {
			t.Fatalf("invalid disabled Brain compiler settings accepted: %+v", cfg.Brain)
		}
	}
}
...
func TestConfigFileOverrideReloadUsesExplicitFile(t *testing.T) {
	...
	if cfg.API.Addr != "127.0.0.1:19998" || cfg.Tool.TimeoutSeconds != 20 {
