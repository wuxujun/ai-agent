...
func TestPlannedStepsForLogIncludesStepAndSafeParameters(t *testing.T) {
	...
	if steps[0].Parameters["query"] != "查教师李松霖的信息" {
		t.Fatalf("query missing from planned step: %#v", steps[0].Parameters)
	}
...
func TestPlannedStepsForLogTruncatesLongValues(t *testing.T) {
	steps := plannedStepsForLog(1, []ActionCall{{Action: "web_search", Parameters: map[string]any{"query": strings.Repeat("界", plannedParameterLogLimit+1)}}})
	query, _ := steps[0].Parameters["query"].(string)
	if len([]rune(query)) != plannedParameterLogLimit+3 || !strings.HasSuffix(query, "...") {
		t.Fatalf("long query was not rune-safe truncated: rune length=%d", len([]rune(query)))
	}
}
